...
export class RadioPanel extends DisplayComponent<RadioPanelProps> {
	...
	public onAfterRender(): void {
		const inputParent = this.inputBarRef.instance.getInputBar().instance;
		...
		const checkParentLoaded: Promise<HTMLDivElement> = new Promise(resolve => {
			const interval = setInterval(() => {
				const inputDiv = inputParent.querySelector('.default-input');
				if (inputDiv !== null) {
			...
			this.subscriber.on('deleteMessage').handle((message) => {
				for (let i = this.messageContainerRef.instance.childNodes.length - 1; i >= 0; i--) {
					const node = this.messageContainerRef.instance.childNodes[i];
					const includesMsg = node.textContent?.includes(message);
					if (includesMsg) {
						node.remove();
						return;
					}
